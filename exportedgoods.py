...
def kishor6():
    ...
    def AddEmployee():
        window = Tk()
        window.title("EXPORTED GOODS INFO")
        ...
        def addemployee():
            ...
            conn = sqlite3.connect(link + db)
            try:
                conn.execute('''CREATE TABLE exportedgoods
            ...
            sql = "INSERT INTO exportedgoods (Product_ID ,product_name,company_name,Date_of_mfg,company_address,owner_details,Date_of_exp ,price ) VALUES (?,?,?,?,?,?,?,?)"
    ...
    def EmployeeInformation():
        window = Tk()
        window.title("exported goods")
        window.geometry("1520x850")
        window.configure('skyblue')
        lbl11 = Label(window, text="GOODS INFO INFORMATION", font=("Algerian", 50))
        ...
        def view():
            cursor = conn.execute("SELECT * FROM exportedgoods")
            details = []
            for row in cursor:
                print(row[1])
    ...
    def EditEmployee():
        ...
        window.geometry("1520x850")
        window.configure(bg='skyblue')
        lbl = Label(window, text="UPDATE DATA OF EXPORTED GOODS", font=("Algerian", 50))
        lbl.grid(column=0, row=0)
        ...
        def EditEmployee():
            ...
            conn = sqlite3.connect(link + db)
            val = (ent1.get(), ent2.get(), ent3.get(), ent4.get(), ent5.get(), ent6.get(), ent7.get(),ent8.get())
            sql = "update exportedgoods set  Product ID = ?,  product Name = ?, company Name = ?, date of mfg = ?, company address = ?, owner details = ?, date of exp= ?, price = ?, Salary = ? where  =Product ID ? "
            conn.execute(sql, val);
            conn.commit()
    ...
    def window1():
        ...
        window.geometry("1520x850")
        window.configure(bg='skyblue')
        lbl = Label(window, text=" EXPORTED GOOD DETAILS", font=("Algerian", 60))
        lbl.grid(column=0, row=0)

        lbl2 = Label(window, text="Add EXPORTED GOODS", font=("Time New Romana Bold", 30))
        ...
        lbl3 = Label(window, text="EXPORTED GOODS INFO", font=("Time New Romana Bold", 30))
        lbl3.grid(column=0, row=3)
        def clicked():
            window.destroy()
            EmployeeInformation()
        ...
        lbl4 = Label(window, text="EDIT EXPORTED GOODS DETAILS", font=("Time New Romana Bold", 30))
